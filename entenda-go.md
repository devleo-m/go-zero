...
# 💡 Regra de Ouro
# Tráfego < 10,000 req/dia       → Node.js está OK
# Tráfego > 100,000 req/dia      → Considere Go seriamente
# Tráfego > 1,000,000 req/dia    → Go é quase obrigatório
...
# typescript
# // user.service.tsexport class UserService {  getUser(id: number) {    return db.query('SELECT * FROM users WHERE id = ?', [id])  }}
# O que acontece:
# Node lê o arquivo .ts
# TypeScript compila para .js
# V8 interpreta o .js
# V8 compila JIT (Just-In-Time) para código de máquina
# Código é executado
# Problemas:
# Overhead da VM (V8)
...
# Go (Compilado)
# go
# // user_service.gotype UserService struct {  db *sql.DB}func (s *UserService) GetUser(id int) (User, error) {  return db.Query("SELECT * FROM users WHERE id = ?", id)}
# O que acontece:
# go build compila TUDO para código de máquina
# Gera um binário (~10-20MB)
# Binário roda DIRETO no processador
# Vantagens:
# Sem overhead de VM
...
# Comparação Visual
# Node.js Deploy:📦 node_modules/ (300MB)📄 package.json📄 .env📄 src/ ├── controllers/ ├── services/ └── models/🏃 Precisa: Node runtime instalado
# Go Deploy:📦 api-server (15MB - binário único)📄 .env (opcional)🏃 Pronto! Roda sozinho

### 6. Exemplos Práticos - Código Real
# Servidor HTTP Simples
# TypeScript (Express):
# typescript
# import express from 'express'const app = express()app.get('/health', (req, res) => {  res.json({ status: 'ok' })})app.listen(3000)
# Go (stdlib):
# go
# package mainimport (    "encoding/json"    "net/http")func health(w http.ResponseWriter, r *http.Request) {    json.NewEncoder(w).Encode(map[string]string{"status": "ok"})}func main() {    http.HandleFunc("/health", health)    http.ListenAndServe(":3000", nil)}
# Diferenças:
...
# Realidade: Go tem 25 palavras-chave (TypeScript tem ~100+). É uma das linguagens mais simples.
# ❌ "Não tem generics"
# Realidade: Go 1.18+ (2022) tem generics. Mas você vai usar pouco.
# ❌ "Error handling é verboso"
# Realidade: if err != nil é mais explícito que try/catch. Você sabe EXATAMENTE onde erros podem acontecer.
# ❌ "Não tem NPM"
# Realidade: Go tem módulos nativos (go.mod). Dependências são versionadas e MUITO mais estáveis que NPM.
# ❌ "Não é produtivo"
# Realidade: Google, Uber, Netflix discordam. Menos bugs = mais produtividade no longo prazo.
